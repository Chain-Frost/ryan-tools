...
def render_scenario_markdown(results: Sequence[ScenarioResult]) -> str:
    ...
    lines = [
        "| Alternative | Crossing | Scenario | Q (m3/s) | HW (m) | TW (m) | Vout max (m/s) | Status |",
        "| --- | --- | --- | ---: | ---: | ---: | ---: | --- |",
    ]
    for result in results:
        ...
        lines.append(
            ...
            + " | ".join(
                (
                    result.alternative_name or "—",
                    result.crossing_name,
...
def render_design_markdown(result: DesignResult) -> str:
    ...
    for index, assessment in enumerate(result.assessments, start=1):
        reason = "—" if assessment.passed else "; ".join(assessment.failure_reasons)
        lines.append(
            f"| {index} | {assessment.candidate.name} | "

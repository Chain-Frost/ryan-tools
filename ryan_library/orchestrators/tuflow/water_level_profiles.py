...
def discover_tuflow_profile_rasters(
    ...
    *,
    ...
    if not results_directory.is_dir():
        raise FileNotFoundError(f"TUFLOW results directory not found: {results_directory}")
    if not target_aeps:
    ...
    for path in results_directory.rglob("*"):
        ...
        if parser.data_type is None or parser.data_type.casefold() != target_result_type.casefold():
            continue
        if parser.aep is None:
            continue
    ...
    return {aep: matches[aep][0] for aep in requested}


def resolve_profile_layer_name(lines_gpkg: Path, requested_layer: str | None) -> str:
    """Resolve a GeoPackage layer using explicit, sole-layer, or stem matching."""
    if not lines_gpkg.is_file():
        raise FileNotFoundError(f"Profile-line GeoPackage not found: {lines_gpkg}")
    layer_table = gpd.list_layers(lines_gpkg)
    available = tuple(str(value) for value in layer_table["name"].tolist())
    if not available:
        raise ValueError(f"GeoPackage contains no readable layers: {lines_gpkg}")

    if requested_layer is not None:
        matches = tuple(name for name in available if name.casefold() == requested_layer.casefold())
        if len(matches) == 1:
            return matches[0]
        raise ValueError(f"Layer {requested_layer!r} not found in {lines_gpkg}. Available layers: {available}")
    if len(available) == 1:
        return available[0]
    ...
    raise ValueError(
        f"Could not choose a unique layer from {lines_gpkg}. Available layers: {available}. "
...
def load_profile_lines(
    ...
    *,
    ...
    """Load profile lines and resolve their CRS against available raster metadata."""
    layer_name = resolve_profile_layer_name(lines_gpkg, requested_layer)
    logger.info("Reading profile lines from {} layer {}", lines_gpkg, layer_name)
    lines = gpd.read_file(lines_gpkg, layer=layer_name)
    if name_field not in lines.columns:
        raise ValueError(f"Field {name_field!r} not found. Available fields: {tuple(lines.columns)}")
    if lines.empty:
        raise ValueError(f"Profile layer is empty: {lines_gpkg}:{layer_name}")
...
def _validate_known_profile_crs(profile_crs: CRS | None) -> None:
    """Validate units when CRS metadata is available."""
    if profile_crs is None:
        return
    if not profile_crs.is_projected:
        raise ValueError(f"Profile sampling requires a projected CRS, received {profile_crs.to_string()}.")
    ...
    if not horizontal_units or not horizontal_units <= {"metre", "meter"}:
        raise ValueError(
            f"Profile distance labels require a metre-based CRS, received units {sorted(horizontal_units)}."
        )
...
def split_profile_line(
    ...
    *,
    ...
    if not isinstance(geometry, MultiLineString):
        raise ValueError(f"Profile {line_name!r} has unsupported geometry type {geometry.geom_type!r}.")
    ...
    if disconnected_handling == "error":
        raise ValueError(
            f"Profile {line_name!r} contains {len(merged.geoms)} disconnected line parts. "
            "Use disconnected_line_handling='separate' to plot every part."
        )
...
def _raster_crs_and_spacing(path: Path, spacing: float | None) -> tuple[CRS | None, float]:
    if not path.is_file():
        raise FileNotFoundError(f"Raster not found: {path}")
    with rasterio.open(path) as source:
        raster_crs = CRS.from_user_input(source.crs) if source.crs is not None else None
        effective_spacing = (
            spacing if spacing is not None else min(abs(float(source.res[0])), abs(float(source.res[1]))) / 2.0
        )
    return raster_crs, effective_spacing


def _read_water_raster_crs(water_rasters: Mapping[str, Path]) -> dict[str, CRS | None]:
...
def _resolve_known_raster_crs(terrain_crs: CRS | None, water_crs: Mapping[str, CRS | None]) -> CRS | None:
    ...
    for name, crs in known[1:]:
        if crs != reference_crs:
            raise ValueError(
                f"CRS mismatch: {reference_name} uses {reference_crs.to_string()}, "
                f"but {name} uses {crs.to_string()}."
            )
...
def run_water_level_profile_workflow(
    ...
    terrain_crs, effective_spacing = _raster_crs_and_spacing(config.terrain_raster, config.spacing)
    ...
    for _, row in lines.iterrows():
        ...
        if raw_name is None or bool(pd.isna(raw_name)):
            raise ValueError(f"Profile has an empty {config.name_field!r} value.")
        line_name = str(raw_name)
        if not line_name.strip():
            raise ValueError(f"Profile has a blank {config.name_field!r} value.")
        ...
        for part_number, line in enumerate(parts, start=1):
            ...
            if normalized_output in reserved_names:
                raise ValueError(f"Duplicate profile output filename: {output_path.name}")
    ...
    for display_name, line, output_path in profile_jobs:
        ...
        figure, axes = plt.subplots(figsize=(config.plot_width_cm / 2.54, config.plot_height_cm / 2.54))
        ...
        try:
            ...
            for aep, raster_path in water_rasters.items():
                ...
                if not np.array_equal(distances, water_distances):
                    raise RuntimeError(f"Sampling stations differ between terrain and {aep} raster.")
                water = interpolate_short_nan_gaps(water, max_gap=config.max_interpolation_gap)
    ...
    logger.success("Created {} water-level profile plots", len(output_paths))

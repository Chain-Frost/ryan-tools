...
class NmxProcessor(MaxDataProcessor):
    ...
    def process(self) -> None:
        ...
        try:
            ...
            self.processed = True
            logger.info(f"Completed processing of NMX file: {self.log_path}")

        except Exception as e:
            logger.error(f"Failed to process NMX file {self.log_path}: {e}")
            self.df = pd.DataFrame()
            return
    ...
    def _extract_and_transform_nmx_data(self) -> None:
        ...
        try:
            ...
            if abnormal_suffix_mask.any():
                ...
                logger.warning(f"Abnormal 'node_suffix' values detected in file {self.log_path}: {abnormal_values}")
                # Exclude these rows
            ...
            if missing_pivot_columns:
                logger.error(
                    f"Missing expected columns after pivoting for file {self.log_path}: {missing_pivot_columns}"
                )
                self.df = pd.DataFrame()
                return
            ...
            logger.error(f"Missing expected columns during extraction for file {self.log_path}: {e}")
            self.df = pd.DataFrame()

        except Exception as e:
            logger.error(f"Unexpected error during NMX data extraction for file {self.log_path}: {e}")
            self.df = pd.DataFrame()

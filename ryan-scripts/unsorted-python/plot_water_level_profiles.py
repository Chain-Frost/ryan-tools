r"""Plot terrain, PFS formation, and TUFLOW water-level profiles.

The input centreline is a 3D alignment. Its Z values are plotted as
``PFS Formation``.

This project uses TUFLOW event names such as AEP5, AEP10, AEP20, AEP50,
and AEP100.

The profile starts at chainage 29.2 km. Raster sampling distances are in
metres and the orchestrator converts them once to kilometre chainage.
"""

from pathlib import Path
from typing import Literal

WRAPPER_VERSION = "2026-08-21.3"
...
LINES_GPKG = Path(
    r"P:\26\RP26171 104 105 EXPLOR DRV HYDRO ASS - ARGCON"
    r"\4 ENGINEERING\11 HYDROLOGY\gis\initial_centreline.gpkg"
)

LINES_LAYER_NAME: str | None = None
NAME_FIELD = "olinetype"
LINES_CRS_IF_MISSING: str | None = None

TERRAIN_TIF = Path(
    r"Q:\BGER\PER\RP20181.554 MLTP 30-37KM DUPLICATION FS - RTIO"
    r"\TUFLOW_MLTP_Duplication\PROJECT_INPUTS\Topo"
    r"\AAM_TPML01_CH16_to_Dingo_1mDEM_MGA94z50.tif"
)

TUFLOW_RESULTS_DIR = Path(
    r"Q:\BGER\PER\RP20181.554 MLTP 30-37KM DUPLICATION FS - RTIO"
    r"\TUFLOW_MLTP_Duplication\results\Dingo_Darren"
    r"\Initial_Version\grids"
)
...
CHAINAGE_START_KM = 29.2

FORMATION_LABEL = "PFS Formation"
FORMATION_COLOR = "black"
FORMATION_LINEWIDTH = 1.4
FORMATION_LINESTYLE = "--"
...
DISCONNECTED_LINE_HANDLING: Literal[
    "error",
    "separate",
] = "error"
...
def main(
    *,
    ...
    config = WaterLevelProfileConfig(
        ...
        formation_linewidth=FORMATION_LINEWIDTH,
        formation_linestyle=FORMATION_LINESTYLE,
        colors=PROFILE_COLORS,
        minor_grid_subdivisions=MINOR_GRID_SUBDIVISIONS,
    )
...
def _parse_cli_arguments() -> CliOptions:
    parser = argparse.ArgumentParser(
        description=(
            "Plot terrain, formation and TUFLOW water levels "
            "along configured profile lines. "
    ...
    parser.add_argument(
        "--name-field",
        help="Attribute used in plot titles and output filenames.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--terrain-raster",
        type=Path,
        help="Terrain elevation GeoTIFF.",
    )
    parser.add_argument(
        "--results-directory",
        type=Path,
    ...
    parser.add_argument(
        "--output-directory",
        type=Path,
        help="Destination for profile PNG files.",
    )
    parser.add_argument(
        "--target-aeps",
        nargs="+",
        help="AEP tokens to plot, such as AEP5 AEP20 AEP100.",
    )
    parser.add_argument(
        "--target-result-type",
        help="Exact TUFLOW raster type, such as h_HR_Max.",
    )
    parser.add_argument(
        "--spacing",
        type=float,
        help="Maximum station spacing in projected CRS units.",
    )
    parser.add_argument(
        "--sampling-method",
        choices=(

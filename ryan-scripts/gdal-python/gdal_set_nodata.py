...
This is the Python equivalent of ``gdal_edit_Set_nodata.bat``. It changes only
the NoData value stored in each selected raster band; it does not rewrite pixel
values. Edit the constants below for double-click/IDE use or pass CLI options.
...
def main(
    *,
    ...
    with setup_logger(console_log_level=console_log_level or CONSOLE_LOG_LEVEL):
        try:
            outputs = set_nodata_in_directory(
                target_directory,
...
def _parse_cli_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Set NoData metadata on matching rasters without changing pixels.")

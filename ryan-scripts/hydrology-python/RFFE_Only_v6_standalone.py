# ryan-scripts/misc-python/RFFE_Only_v6_standalone.py
# Standalone RFFE batch extractor (no ryan_library dependency)
#
# Required pip install:
#   python -m pip install pandas requests loguru
#
# Run:
...
A comma-separated values file with a header row and the following columns:
    ...
    OutletX     (float) Longitude of the catchment outlet, in decimal degrees.
    OutletY     (float) Latitude of the catchment outlet, in decimal degrees.
    CentroidX   (float) Longitude of the catchment centroid, in decimal degrees.
    CentroidY   (float) Latitude of the catchment centroid, in decimal degrees.
...
def setup_logger(console_log_level: str = "INFO", log_file: str | None = None) -> None:
    """Configure loguru sinks for console and optional file output."""
    logger.remove()
    logger.add(
        sink=sys.stdout,
        level=console_log_level,
        format="<green>{time:HH:mm:ss}</green> | <level>{level}</level> | {module}:{function}:{line} - {message}",
        colorize=True,
        backtrace=True,
        diagnose=False,
        enqueue=False,
...
class Catchment:
    ...
    def _extract_js_array(html: str, var_name: str) -> Sequence[Record]:
        """Extract `var_name = [ ... ]` JS array and parse to Python records."""
    ...
    def parse(self, html: str) -> tuple[DataFrame, DataFrame]:
        """Return (results_df, all_catchment_results_df)."""
        results: Sequence[Record] = self._extract_js_array(html=html, var_name="results")
        all_results: Sequence[Record] = self._extract_js_array(html=html, var_name="allCatchmentResults")
        df_results = pd.DataFrame(data=results)
        df_all = pd.DataFrame(data=all_results)
        logger.info(f"[{self.name}] Parsed {len(df_results)} results, {len(df_all)} allCatchmentResults")
    ...
    def process(self, session: requests.Session, out_dir: Path) -> tuple[DataFrame, DataFrame, str | None]:
        ...
        df_results, df_all = self.parse(resp.text)

        if not df_results.empty:
            df_results["Catchment"] = self.name
        if not df_all.empty:
            df_all["Catchment"] = self.name

        return df_results, df_all, None
...
def main() -> int:
    ...
    parser.add_argument("--input-dir", type=Path, help="Folder with input_catchments.csv")
    parser.add_argument("--output-dir", type=Path, help="Folder for CSV outputs")
    parser.add_argument(
        "--log-level",
    ...
    try:
        df_in: DataFrame = pd.read_csv(filepath_or_buffer=csv_fp)  # type: ignore[no-any-return]
    except Exception as ex:
    ...
    catchments: list[Catchment] = [
        ...
        for idx, rec in enumerate(iterable=df_in.to_dict(orient="records"), start=1)  # type: ignore[arg-type]
    ]
    ...
    with requests.Session() as sess:
        for idx, ct in enumerate(iterable=catchments, start=1):
            ...
            if err:
                ...
                logger.warning(f"FAIL {ct.name}: {err}")
                continue
            if not df_r.empty:
                all_res.append(df_r)
            if not df_a.empty:

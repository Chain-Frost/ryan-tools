...
def calculate_central_tendency_statistics(
    ...
    """
    max_central_value: float = -float("inf")
...
def analyze_hydrograph(rline: tuple[str, str, int, str, str], qcheck_list: list[float | int]) -> pd.DataFrame:
    """
...
def run_processing_pipeline() -> None:
    ...
    if not files_df.empty:
        ...
        qcheck: list[int] = list(range(1, 10)) + list(range(10, 100, 2)) + list(range(100, 2100, 10))
        ...
        if not df.empty:
            ...
            output_prefix: str = f"{date_time_string}_durex_5c-zeroes"
            ...
            date_time_string = datetime.now().strftime("%Y%m%d-%H%M")
            save_as = Path(f"{date_time_string}_QvsTexc_v5c-zeroes.csv")

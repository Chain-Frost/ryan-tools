...
def get_parameters() -> dict[str, str | list[str] | dict[str, list[str]] | dict[str, int] | bool | float | None]:
...
def main() -> None:
    ...
    if params.get("priority_order") is not None:
        ...
        sorted_keys = sorted(
            keys, key=lambda x: priority_order.index(x) if x in priority_order else len(priority_order)
        )

...
def stats(freqdb, statcol, tpcol, durcol):
    """
    This function extracts min, max, and median volume exceedances for each duration group.
    It returns the corresponding critical storm parameters and volume statistics.
    """
    median = -9999
    for durgrp in freqdb.groupby(by=durcol):
        ...
        current_median = ensemblestat[statcol].iloc[medianpos]
...
def process_hydrographCSV(params_dict):
    ...
    # Initialize the DataFrame to store volume exceedance data
    durexcdb = pd.DataFrame(
        columns=[
            ...
            "Location",
            "ThresholdFlow",
            "Volume_Exceeding",  # Updated column name
            "Runcode",
            "TrimRC",
        ]
    )

    # Read the hydrograph CSV
    hydrographs = read_csv(
        filepathname=csv_read, separator=",", skip_row=[0], header_row=0, skip_cols=1
    )

    # Determine the timestep (assuming time is in consistent units, e.g., hours)
    timestep = hydrographs["Time"].diff().iloc[1]

    for qch in qcheckList:
        # Select flows that exceed the threshold
        exceedances = hydrographs[hydrographs > qch]

        # Calculate flow exceedance rate: flow - qch
        flow_exceedance = exceedances - qch

        # Calculate volume exceedance for each timestep
        volume_exceedance = flow_exceedance * timestep * 3600

        # Sum volume exceedances per location
        volume_sum = volume_exceedance.sum()[1:]  # Exclude the Time column

        # Get the list of locations where exceedance occurred
        locations = volume_sum[volume_sum > 0].index.tolist()

        # Get the corresponding volume exceedances
        volumes = volume_sum[volume_sum > 0].values.tolist()

        # Create a dictionary for the current qch
        dictionary = {
            ...
            "Volume_Exceeding": volumes,  # Store volume exceedance
...
if __name__ == "__main__":
            ...
            for flowgrp in locgrp[1].groupby(by="ThresholdFlow"):
                for AEPgrp in flowgrp[1].groupby(by="AEP"):
                    result = [TrimRC[0], locgrp[0], flowgrp[0], AEPgrp[0]] + stats(
                        AEPgrp[1], "Volume_Exceeding", "TP", "Duration"
                    )
    ...
    save_as = f"{DateTimeString}_VolvsTexc.csv"
...
if __name__ == "__main__":
    ...
    pltfolder = working_dir + r"\plot_volume_exceedance"
    print("")
    print("Now plotting")

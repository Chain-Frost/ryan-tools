...
def _get_scalar(alpha_0: npt.NDArray[np.float64], q_coords: npt.NDArray[np.float64] | None = None) -> np.float64:
    if q_coords is None:
        scalar = np.einsum("i->", alpha_0)
    else:
        scalar = np.einsum("i,i->", alpha_0, (q_coords * q_coords).sum(0))
    return scalar  # type: ignore


def _get_q_matrix(quaternions: npt.NDArray[np.float64]) -> npt.NDArray[np.float64]:
    q_matrix = [
...
def _get_w_matrix(quaternions: npt.NDArray[np.float64]) -> npt.NDArray[np.float64]:
    w_matrix = [
        [[q[3], q[2], -q[1], q[0]], [-q[2], q[3], q[0], q[1]], [q[1], -q[0], q[3], q[2]], [-q[0], -q[1], -q[2], q[3]]]
        for q in quaternions
    ]
...
def _get_abc_matrices(
    alpha_0: npt.NDArray[np.float64], m1: npt.NDArray[np.float64], m2: npt.NDArray[np.float64] | None = None
...
def _get_blc_matrix(
    b_matrix: npt.NDArray[np.float64], lambda_i: float, c_matrix: npt.NDArray[np.float64]
) -> npt.NDArray[np.float64]:
...
def _get_d_matrix(
    li: float, cs: np.float64, am: npt.NDArray[np.float64], blcm: npt.NDArray[np.float64]
) -> npt.NDArray[np.float64]:
    d_matrix = 2 * li * am + (1 / cs) * (blcm.T @ blcm)
    return d_matrix


def _get_r_quat(d_matrix: npt.NDArray[np.float64]) -> tuple[np.intp, npt.NDArray[np.float64]]:
    eigvals, eigvects = np.linalg.eig(d_matrix)
    beta_1: intp = np.argmax(eigvals)
    r_quat = eigvects[:, beta_1]
    return beta_1, r_quat  # type: ignore


def _get_lambda_next(
    am: npt.NDArray[np.float64],
    bs: np.float64,
    bm: npt.NDArray[np.float64],
    cs: np.float64,
    cm: npt.NDArray[np.float64],
    rq: npt.NDArray[np.float64],
) -> float:
    expr_1 = rq.T @ am @ rq
    expr_2 = (1 / cs) * (rq.T @ bm.T @ cm @ rq)
    expr_3 = (1 / cs) * (rq.T @ cm.T @ cm @ rq)
    lambda_next = (expr_1 - expr_2) / (bs - expr_3)
    return float(lambda_next)  # type: ignore
...
def _get_solution(
    ...
    li: float,
    i: int,
) -> tuple[npt.NDArray[np.float64], npt.NDArray[np.float64], np.intp, npt.NDArray[np.float64], float, int]:
    blc_matrix: NDArray[float64] = _get_blc_matrix(b_matrix=bm, lambda_i=li, c_matrix=cm)
    d_matrix: NDArray[float64] = _get_d_matrix(li, cs, am, blc_matrix)
...
def _get_r_matrix(r_quat: npt.NDArray[np.float64]) -> npt.NDArray[np.float64]:
    r_w_matrix = _get_w_matrix(quaternions=np.array([r_quat]))[0]
    r_q_matrix = _get_q_matrix(quaternions=np.array([r_quat]))[0]
    r_matrix = (r_w_matrix.T @ r_q_matrix)[:3, :3]
    return r_matrix


def _get_s_quat(
    c_scalar: np.float64, blcm: npt.NDArray[np.float64], r_quat: npt.NDArray[np.float64]
) -> npt.NDArray[np.float64]:
    s_quat = 1 / (2 * c_scalar) * (blcm @ r_quat)
    return s_quat  # type: ignore


def _get_t_vector(r_quat: npt.NDArray[np.float64], s_quat: npt.NDArray[np.float64]) -> list[npt.NDArray[np.float64]]:
    r_w_matrix = _get_w_matrix(np.array([r_quat]))[0]
    t_vector = [2 * (r_w_matrix.T @ s_quat)[:3]]  # type: ignore
    return t_vector
...
def find(
    source_points: Sequence[Sequence[float]] | npt.NDArray[np.float64],
    target_points: Sequence[Sequence[float]] | npt.NDArray[np.float64],
    alpha_0: Sequence[float] | npt.NDArray[np.float64] | None = None,
    scale: bool = True,
    lambda_0: float = 1.0,
    ...
    alpha_0_arr: npt.NDArray[np.float64]
    if alpha_0 is None:
        alpha_0_arr = np.ones(n, dtype=np.float64)
    else:
        alpha_0_arr = np.array(alpha_0, dtype=np.float64)

    if alpha_0_arr.ndim != 1:
        err_msg = "alpha_0 array must have dimension = 1."
        raise ValueError(err_msg)

    if alpha_0_arr.shape != (n,):
        err_msg = "There are not as many alpha_0 coefficients as " "control points."
        raise ValueError(err_msg)

    lambda_0_float = float(lambda_0)
    ...
    source_q_coords: NDArray[float64] = np.concatenate((source_coords, np.zeros((1, n))))

    target_q_coords: NDArray[float64] = np.concatenate((target_coords, np.zeros((1, n))))

    b_scalar: float64 = _get_scalar(alpha_0=alpha_0_arr, q_coords=source_q_coords)

    c_scalar: np.float64 = np.einsum("i->", alpha_0_arr)  # type: ignore

    q0_w_matrix: NDArray[float64] = _get_w_matrix(quaternions=source_q_coords.T)

    qt_q_matrix: NDArray[float64] = _get_q_matrix(quaternions=target_q_coords.T)

    a_matrix: NDArray[float64] = _get_abc_matrices(alpha_0=alpha_0_arr, m1=q0_w_matrix, m2=qt_q_matrix)

    b_matrix: NDArray[float64] = _get_abc_matrices(alpha_0=alpha_0_arr, m1=qt_q_matrix)

    c_matrix: NDArray[float64] = _get_abc_matrices(alpha_0=alpha_0_arr, m1=q0_w_matrix)

    lambda_i, i = lambda_0_float, 1

    blc_matrix, _d_matrix, _beta_1, r_quat, lambda_i, i = _get_solution(
        am=a_matrix, bs=b_scalar, bm=b_matrix, cs=c_scalar, cm=c_matrix, scale=scale, li=lambda_i, i=i
    )

    r_matrix: NDArray[float64] = _get_r_matrix(r_quat)

    s_quat: NDArray[float64] = _get_s_quat(c_scalar, blc_matrix, r_quat)

    t_vector: np.ndarray[tuple[int, int], np.dtype[float64]] = np.array(_get_t_vector(r_quat, s_quat)).reshape(3, 1)

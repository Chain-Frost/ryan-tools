...
def mock_aggregated_df():
    return pd.DataFrame(
        {
            "aep_text": ["1%", "1%", "1%", "1%"],
            "duration_text": ["1h", "1h", "2h", "2h"],
            "tp_text": ["tp1", "tp2", "tp1", "tp2"],
            "tp_numeric": [1, 2, 1, 2],
            "trim_runcode": ["run1", "run1", "run1", "run1"],  # Same runcode to allow grouping/comparison
            "Chan ID": ["C1", "C1", "C1", "C1"],
            "Q": [10.0, 12.0, 14.0, 16.0],
            "V": [1.0, 1.2, 1.4, 1.6],
            "DS_h": [5.0, 5.1, 5.2, 5.3],
            "US_h": [6.0, 6.1, 6.2, 6.3],
        }
    )
...
def test_find_culvert_aep_dur_mean_success(mock_aggregated_df):
    ...
    assert "tp_text" not in result.columns
    assert "mean_tp_numeric" not in result.columns

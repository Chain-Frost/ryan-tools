...
def test_median_stats_for_group_odd():
    df = make_df([1, 2, 3], ["A", "B", "C"], ["5", "5", "5"])
...
def test_median_stats_for_group_even():
    df = make_df([4, 1, 3, 2], ["A", "B", "C", "D"], ["1", "1", "1", "1"])
    stats = _median_stats_for_group(df, "val", "tp", "dur")
...
def test_median_stats_for_group_zeros():
    df = make_df([0, 0, 0], ["A", "B", "C"], ["d", "d", "d"])
    stats = _median_stats_for_group(df, "val", "tp", "dur")
...
def test_median_calc_multiple_groups():
    df = pd.concat(
        [
...
def test_median_calc_ties_choose_first():
    df = pd.concat(
        [
            make_df([2, 2], ["A", "B"], ["5", "5"]),
...
def test_median_calc_single_group():
    df = make_df([7, 9, 8], ["A", "B", "C"], ["1", "1", "1"])
    max_stats, stats_list = median_calc(df, "val", "tp", "dur")

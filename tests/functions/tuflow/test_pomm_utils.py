...
class TestAggregation:
    ...
    def test_find_aep_dur_max(self, sample_df: DataFrame) -> None:
        ...
        res: DataFrame = pomm_utils.find_aep_dur_max(aggregated_df=sample_df)
        assert len(res) == 3
        # Check 1% 1h group
        row1: DataFrame = res[(res["aep_text"] == "1%") & (res["duration_text"] == "1h")]
        assert not row1.empty
        assert row1["AbsMax"].iloc[0] == 20.0
        assert row1["count_TP"].iloc[0] == 2  # Group size

    def test_find_aep_max(self, sample_df: DataFrame) -> None:
        """Test finding max per AEP group."""
        # First get aep_dur_max
        aep_dur_max: DataFrame = pomm_utils.find_aep_dur_max(aggregated_df=sample_df)
        ...
        assert not row1.empty
        assert row1["AbsMax"].iloc[0] == 20.0
        assert row1["count_TP_aep"].iloc[0] == 2  # 2 durations in 1% group
    ...
    def test_find_aep_dur_median(self, mock_median, sample_df):
        ...
        res: DataFrame = pomm_utils.find_aep_dur_median(aggregated_df=sample_df)
    ...
    def test_find_aep_median_max(self, sample_df: DataFrame) -> None:
        ...
        res: DataFrame = pomm_utils.find_aep_median_max(aep_dur_median=median_df)
        assert len(res) == 1
        assert res["MedianAbsMax"].iloc[0] == 60.0
        assert res["count_TP_aep"].iloc[0] == 2


class TestHelpers:
    def test_ordered_columns(self) -> None:
        """Test column ordering logic."""
        df = pd.DataFrame(columns=["A", "B", "C", "D", "E"])
        groups: list[list[str]] = [["A", "C"], ["B"]]
        info: list[str] = ["E"]
        # Expected: A, C, B, D (remaining), E
        res: list[str] = pomm_utils._ordered_columns(df=df, column_groups=groups, info_columns=info)
...
class TestReporting:
    ...
    def test_save_to_excel(self, mock_exporter, tmp_path):
        ...
        df = pd.DataFrame({"A": [1]})
        pomm_utils.save_to_excel(aep_dur_max=df, aep_max=df, aggregated_df=df, output_path=tmp_path / "out.xlsx")
...
class TestMeanAggregation:
    @pytest.fixture
    def sample_median_df(self) -> DataFrame:
        return pd.DataFrame(
            data={
                "aep_text": ["1%", "1%"],
                "duration_text": ["1h", "2h"],
...
class TestReportWrappers:
    ...
    def test_save_peak_report_median(self, mock_dur, mock_max, mock_save, tmp_path):
        ...
        pomm_utils.save_peak_report_median(aggregated_df=pd.DataFrame(), script_directory=tmp_path, timestamp="2025")
    ...
    def test_save_peak_report_mean(self, mock_dur, mock_max, mock_save, tmp_path):
        ...
        pomm_utils.save_peak_report_mean(aggregated_df=pd.DataFrame(), script_directory=tmp_path, timestamp="2025")
...
class TestRLLQmxNormalization:
    ...
    def test_aggregated_from_paths_normalization(self, mock_combine) -> None:
    ...
    @patch("ryan_library.functions.tuflow.pomm_utils.combine_df_from_paths")
    def test_aggregated_from_paths_mixed(self, mock_combine) -> None:
